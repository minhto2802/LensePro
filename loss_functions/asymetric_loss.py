...
class NCELoss(nn.Module):
    ...
    def forward(self, pred, labels, reduction='none'):
...
class AGCELoss(nn.Module):
    def __init__(self, num_classes=10, a=1, q=2, eps=eps, scale=1., reduction='none'):
...
class NLNL(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        labels_neg = (labels.unsqueeze(-1).repeat(1, self.ln_neg)
                      + torch.LongTensor(len(labels), self.ln_neg).cuda().random_(1,
...
class NCEandAGCE(torch.nn.Module):
    def __init__(self, alpha=1., beta=1., num_classes=10, a=3, q=1.5, reduction='mean'):
    ...
    def forward(self, pred, labels, reduction='none', *args, **kwargs):
        reduction = self.reduction if reduction is 'none' else reduction
        return self.nce(pred, labels, reduction) + self.agce(pred, labels, reduction)
...
class NCEandAUE(torch.nn.Module):
    ...
    def forward(self, pred, labels, reduction='none'):
        return self.nce(pred, labels, reduction) + self.aue(pred, labels, reduction)
