...
class IsoMaxPlusLossFirstPart(nn.Module):
    ...
    def __init__(self, num_features, num_classes, temperature=1.0, const_init=0):
        ...
        self.prototypes = nn.Parameter(torch.Tensor(num_classes * 1 * 1, num_features))
    ...
    def forward(self, features):
        ...
        # distances = distances.view(distances.size(0), self.num_classes, -1, 4).mean(dim=-1).min(dim=-1)[0]
        # distances = distances.view(distances.size(0), self.num_classes, -1).mean(dim=-1)
        # distances = distances.view(distances.size(0), self.num_classes, -1).min(dim=-1)[0]
        logits = -distances
