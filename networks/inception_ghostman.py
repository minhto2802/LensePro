...
class Inception(nn.Module):
    def __init__(self, in_channels, n_filters, kernel_sizes=[9, 19, 39], bottleneck_channels=32, activation=nn.ReLU(),
                 return_indices=False, num_classes=2):
        """
...
class InceptionTime(nn.Module):
    def __init__(self, in_channels: int, num_pred_classes: int = 2,
                 base_channels=32, bottleneck_channels=32):
        super().__init__()
        ...
        self.feat_extractor = nn.Sequential(
            ...
            Flatten(out_features=base_channels * 4 * 1),
        )
        self.classifier = nn.Sequential(
            nn.Linear(in_features=4 * base_channels * 1,
...
def main():
    ...
    net = InceptionTime(in_channels, pred_classes)
