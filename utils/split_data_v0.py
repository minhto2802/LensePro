...
def merge_split(input_data, random_state=0, val_size=.4, verbose=False):
    ...
    for set_name in ['train', 'val', 'test']:
        gs = gs + list(input_data[f"GS_{set_name}"])
        pid = pid + list(input_data[f"PatientId_{set_name}"])
        inv = inv + list(input_data[f"inv_{set_name}"])
    ...
    split_iterator = GroupShuffleSplit(test_size=val_size, n_splits=20,
                                       random_state=random_state).split(df1, groups=df1['pid'])
    train_ind, test_ind = get_valid_split(split_iterator, np.array(inv) > 0)

    df1 = df1.assign(group='train')
    df1.loc[test_ind, 'group'] = 'test'
    df1 = df1.sort_values(by='pid')

    pid_tv = {
        'train': df1[df1.group == 'train'].pid.unique(),
        'test': df1[df1.group == 'test'].pid.unique(),
    }
    # Merge train - val - test
    keys = [f[:-5] for f in input_data.keys() if 'test' in f]
    merge = {}
    for k in keys:
        if isinstance(input_data[f'{k}_train'], list):
            merge[k] = input_data[f'{k}_train'] + input_data[f'{k}_val'] + input_data[f'{k}_test']
        else:
            merge[k] = np.concatenate([input_data[f'{k}_train'], input_data[f'{k}_val'], input_data[f'{k}_test']],
                                      axis=0)
...
def merge_train_val(input_data):
    # Merge train - val
    keys = [f[:-4] for f in input_data.keys() if 'val' in f]
    merge = {}
    for k in keys:
        if isinstance(input_data[f'{k}_train'], list):
            merge[f'{k}_train'] = input_data[f'{k}_train'] + input_data[f'{k}_val']
        else:
            merge[f'{k}_train'] = np.concatenate([input_data[f'{k}_train'], input_data[f'{k}_val']], axis=0)
        merge[f'{k}_test'] = input_data[f'{k}_test']
...
def merge_split_train_val(input_data, random_state=0, val_size=.4, verbose=False):
    ...
    """
    # merge train-val then randomize patient ID to split train-val again
    merge, keys = merge_train_val(input_data)
    ...
    for set_name in ['train', 'val']:
        for k in keys:
            target[f'{k}_{set_name}'] = []
    # Re-split data into two sets based on randomized patient ID
    for i, pid in enumerate(merge['PatientId_train']):
        for set_name in ['train', 'val']:
            for k in keys:
                k_target = f'{k}_{set_name}'
                if pid in pid_tv[set_name]:
                    target[k_target].append(merge[f'{k}_train'][i])
    # Assign to original input data after finishing creating a new one
    for set_name in ['train', 'val']:
        for k in keys:
            k_target = f'{k}_{set_name}'
            input_data[k_target] = target[k_target]
            if isinstance(merge[f'{k}_train'], np.ndarray):
                input_data[k_target] = np.array(target[k_target]).astype(merge[f'{k}_train'].dtype)
